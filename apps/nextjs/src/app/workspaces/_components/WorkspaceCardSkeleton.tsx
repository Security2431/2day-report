...
const WorkspaceCardSkeleton: React.FC<Props> = ({ pulse = true }) => {
  return (
    <div className="flex h-64 max-w-sm flex-col items-center justify-center gap-3 overflow-hidden rounded-xl border border-white shadow-lg transition-all  hover:scale-[101%] hover:bg-[#2e026fff]">
      <picture
        className={classNames("h-12 w-12 rounded-full bg-pink-400", {
          "animate-pulse": pulse,
        })}
      />
      <h2
      ...
      <span
        className={classNames("w-1/3 rounded bg-current text-sm ", {
          "animate-pulse": pulse,
        })}
      >
        &nbsp;
      ...
      >
        <span
          className={classNames("w-1/2 rounded bg-current text-sm ", {
            "animate-pulse": pulse,
          })}
        >
          &nbsp;
